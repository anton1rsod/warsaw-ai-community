...
- [ ] Reach **`/admin/invite`** — as of **v0.12.1** there is a front door: click your **`@handle ▾`** account dropdown → **"admin console"** → `/admin` → **Invite**. Direct URL bookmark as fallback: `https://warsaw-ai-community-platform.vercel.app/admin/invite`. Scroll to the **"Meeting invite"** section (the *second* form; the top "Mint invitation" form is the personal single-use one — not this).
