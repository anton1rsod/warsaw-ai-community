...
## ★ Done means
5 phases green (tsc + lint + test + `pnpm e2e --retries=2`); `H138`–`H150` grep-verified; `lib/persona.ts` (+`lib/persona-editor.ts`) added to the **strict-list 100%** in STATE; `CHANGELOG.md` `[0.11.1]`; PR open + CI green; **HOLD for the 6/11 retro before merge**; then tag + STATE flip + prod smoke + memory + next handoff.

## ★ Anti-patterns
- Don't squash-merge before the 6/11 retro.
