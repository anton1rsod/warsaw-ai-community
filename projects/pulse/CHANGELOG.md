...
- `tests/unit/workflow.test.ts` — 7 structural invariants over the YAML (queue-not-cancel, all crons `:30`, push-path-filter + dispatch, `[skip ci]`, two tokens, failure notify, write-back guards a not-yet-created file). 97 tests total (+7); coverage 99.66% lines / 90.84% branches (gate ≥80%).

### Fixed
- Commit-back steps guard `git add` with `[ -f ]` — the generated artifacts (`notion-index.json`, `tasks-snapshot.json`) don't exist until a live run, and `git add` of a missing pathspec is a fatal exit-128 under `bash -e`, so without the guard every dormant run failed at the commit step. Caught on the first real post-merge run (structural tests validate YAML shape, not shell runtime).
