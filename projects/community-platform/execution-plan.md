...
**§9.14 Phase 3 (in-execution 2026-05-01) — `lib/meetings.ts` regex fixes.**
The plan's Task 3.5 has two latent regex bugs that block execution. (a) `parseAttendees` uses `/^##\s+Attendees\s*$([\s\S]*?)(?=^##\s|\z)/m` — `\z` is a Perl/PCRE end-of-string anchor that does NOT exist in JavaScript regex (which uses `$` with `/m` for end-of-line, requiring a lookahead trick for true end-of-string). Replaced with `$(?![\s\S])` to assert true end-of-string under the `/m` flag. (b) `readMeeting` matches the bare `slug` (e.g. `2026-04-24`) against `FILE_RE = /^(\d{4}-\d{2}-\d{2})\.md$/` — the trailing `\.md$` anchor never matches a bare slug, so `readMeeting` always returned `null`. Added a dedicated `SLUG_RE = /^(\d{4}-\d{2}-\d{2})$/` for slug validation in `readMeeting`. Both bugs were caught by the plan's own test cases during TDD red phase; fixes ship in commit `6c4b1a1`.

**§9.15 Phase 3 (in-execution 2026-05-01) — snapshot script throws on null instead of casting.**
The plan's Task 3.1 + 3.3 snapshot extensions use `(await readProject(REPO_ROOT, p.slug))!` / `as Promise<ProjectDetail>` non-null assertions when mapping over slug summaries inside the snapshot script. Both `readProject` and `readDecision` return `T | null` to handle race conditions (slug listed by `listProjects`/`listDecisions` but file deleted before the second read, or `_`-prefixed slug somehow making it through). The casts hide the null branch from TypeScript and emit a `null` entry typed as non-null into the JSON snapshot — pages then crash at request time when accessing `.slug`/`.body` on the null. Phase 3 implementation throws an explicit `Error("[snapshot] readX returned null for slug \"…\" — listX/readX contract violation")` instead, surfacing the contract violation immediately at build time. Ships in `b4475f0` (the batched reviewer-fix commit).

**§9.16 Phase 5 (Anton-approved 2026-05-01) — `/this-week` ships with `dynamic = "force-dynamic"` instead of `revalidate = 60`.**
The plan's Task 5.5 sets `export const revalidate = 60;` on `/this-week`. Phase 5 implementation ships with `export const dynamic = "force-dynamic";` instead. Reasons: (a) ISR + bot commit propagation + 60s cache + SHA-conflict resolution interact in subtle ways (per execution-plan §6.3 risk register entry); the v0.1 spec doesn't require ISR and we'd rather verify the timing operationally first; (b) the E2E mock store on `globalThis` leaks across cache windows when ISR is on, which the dynamic path avoids. A later iteration (Phase 7+ once contributions counter forces us to revisit caching) can re-introduce `revalidate` once the timing is understood. The bot's commit propagation latency (10–60s) is the constraint that ISR was meant to absorb; for v0.1, the cost of a fresh fetch on every page hit is ~200ms (one Contents API + ≤19 file fetches in parallel) — acceptable within the 19-member roster.

**§9.17 Phase 5 (in-execution 2026-05-01) — E2E mock store on `globalThis`.**
Plan's Task 5.6 puts the in-memory mock state in module-scope variables. In Next 16 dev mode, `"use server"` files (server actions) bundle into a separate module graph from server components and route handlers. Each consumer of `_test-status-store.ts` can therefore receive a different module instance — server actions write to instance A, the page reads from instance B, and the reset endpoint clears instance C. Symptom: post + edit/delete tests pass in isolation but fail in sequence because the page never sees what the action wrote. Fix: state lives on `globalThis.__waicMockStatusStore` so all instances share. This is a Next 16 dev-mode quirk; production builds may behave differently but production never imports `_test-status-store.ts` (gated by `NODE_ENV !== "production"` + `NEXT_PUBLIC_E2E_MODE === "1"` checks).
...
**§9.24 Phase v0.4 A.3.2 (in-execution 2026-05-18) — H68 enforcement collapses to existing test (no separate script + CI step).**
v0.4.0-plan.md Task A.3.2 prescribed a NEW `scripts/validate-persona-folders.ts` + `.github/workflows/ci.yml` step. The plan's regex (`/\[(?:[^\]]+)\]\(\.\/([a-z0-9-]+)\.md\)/g`) was designed to parse `community/members/roster.md` for `[Name](./slug.md)` markdown links — but the actual roster.md uses GitHub-handle table cells (`@anton1rsod`), not slug-link patterns, so the regex matches ZERO slugs. The A.3.2 implementer (commit `538f304`) substituted a filename-based check (list `community/members/*.md` filenames as slugs) — sound logic, but it enforces a STRICTER invariant ("every persona folder has a consented profile file") than H68 was meant to express. Result: 3 pre-existing persona folders (heorhii-k, maksym-pavlenko, mark-spasonov) flag as orphans on every CI run because their members haven't gone through consent flow yet. The chat-10 Option D forward-defense test at `tests/unit/persona-folder-slug-consistency.test.ts` (PR #7) ALREADY implements the correct H68 invariant (folder name == slugify(frontmatter.display_name); self-consistent). That test runs in `pnpm test` (CI gate) and passes for all committed persona folders. **Resolution:** A.3.2 reverted as commit `214a602`; backfilled the existing test's describe block from `describe("persona folder ↔ slug consistency", ...)` to `describe("H68: persona folder ↔ slug consistency", ...)` so the hardening grep at chat-25 closeout returns ≥1 hit on `H68:`. No separate script + CI step needed.

**§9.23 Phase v0.4 A.2.2 (in-execution 2026-05-18) — `tests/unit/h-v0-4.test.ts` renamed to `.test.tsx`.**
Phase A.0.2 (commit `569a7cb`) created the aggregate hardening test as `h-v0-4.test.ts` (node env per vitest's environmentMatchGlobs — line 8 only maps `.test.tsx` → jsdom). H64 + H59 + H67 + H56 + H57 describes all worked in node env because they used `readFileSync` (H64/H59) + helper-fn imports (H57). H56 was implemented as a static source-grep (§9.x deviation in A.2.1) to avoid the RTL-in-node-env issue. A.2.2 H62 test requires `render(await CalendarPage(...))` (RTL) which CANNOT run in node env. Rather than refactor H62 into a static check, the A.2.2 implementer renamed the file `h-v0-4.test.ts` → `h-v0-4.test.tsx` (commit `58ef8aa`) — vitest now applies jsdom to all blocks. Existing tests (H64/H59/H56/H57) remain green under jsdom. Subsequent tasks (A.2.3 H62-style if needed, A.0.4 future H59 amendments) should target `h-v0-4.test.tsx` (not `.test.ts`). This supersedes §9.19's blanket `.test.ts` direction for the aggregate hardening file specifically — component tests under `tests/unit/components/` remain `.test.tsx` per §9.21; standalone-helper tests remain `.test.ts`.
