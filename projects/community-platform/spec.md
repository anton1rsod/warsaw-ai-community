...
## §19.1 v0.9.1 — Forms/admin warm reskin + write-path E2E + E2E hygiene (chat-47 / 2026-05-27)

Finishes the §19 rollout on the stateful **write/admin** surfaces v0.9.0 deferred, backfills the authenticated **write-path E2E** (RSVP + Thanks), and cleans the **stale E2E specs** v0.9 surfaced. **Application, not invention** — the warm system + reader recipe (`v0.9.0-plan.md` C1–C5) are frozen; the canonical form precedent (`StatusEditor`) shipped in v0.9.0. `dark:` in `app/` = 0 (H98 holds); the remaining `neutral-*`/`gray-*`/`rounded border` scaffolding is only on the deferred forms/admin files. **Design (canonical):** `docs/specs/2026-05-27-community-platform-v0-9-1-forms-admin-design.md`. **Plan:** `projects/community-platform/v0.9.1-plan.md` (via `superpowers:writing-plans`).

### Scope (complete rollout — Anton, chat-47) — 6 phases, subagent-driven, one implementer per phase

1. **Shared contracts + `Pill`** — add `disabled:opacity-50` (BASE) + a `danger` variant to `Pill`; document **C7** (form-field recipe = `StatusEditor` verbatim), **C8** (modal recipe), **C9** (metric-tile + table recipe); refactor `StatusEditor` (solid+dashed) + `GdprPanel` (solid+danger) to consume `Pill` and stop hand-rolling its look. Touches the shipped `Pill` → re-smoke the 4 hero surfaces.
2. **Member forms** — `/me/edit`+`ProfileEditor` (incl. `.prose-warm` preview, C2, replacing the no-op `prose prose-neutral`), `/consent`+`ConsentClient`+`ConsentModal` (C8 — native `<dialog>` upgrade), `/onboard`+`OnboardForm`+`/onboard/error`, `/no-access`.
3. **Admin** — `/admin/health` (C9 tile + tokenized trend table), `/admin/invite`+`InviteForm`+`InviteUrlDisplay`, `/admin/events/new`+`EventForm` (C7 across many fields — heaviest single reskin at 272 LOC).
4. **Write-path E2E mocks** — `app/actions/_test-rsvp-store.ts` + `_test-thank-store.ts` (in-memory `Map` on `globalThis`, `nextSha`/`resolveAuthState`/`reset`, coverage-excluded) + a `fromMock` fork at the top of `rsvpEvent`/`thankStatus` guarded by `!isProductionRuntime() && isE2EMode()` + `api/test-reset-{rsvp,thank}` routes; un-skip `e2e/v0-9-write-paths.spec.ts` 5.4a (RSVP toggle) + 5.4b (Thanks). Mirrors the `_test-status-store` precedent exactly (incl. the Next-16 `globalThis` module-graph hop).
5. **E2E hygiene** — rewrite `smoke`/`auth`×3/`consent`/`members:22` assertions to the current ADR-0012/0014 public-discovery routing (obsolete "everything → `/login`"); `test.skip` the prod-only `v0-4-shell`×3 (H56 `Cache-Control` is Vercel-edge-only; v0.6 hero markup) when `PLAYWRIGHT_BASE_URL` ≠ prod. No coverage deleted — made conditional on its valid target.
6. **Followup + closeout** — restore `<code>` mono on the `members/[slug]` empty-state path/slug; final `dark:` grep-guard (still 0); per-surface source-scan tests; CHANGELOG + STATE flip + tag `community-platform-v0.9.1`.

### Decisions (locked chat-47 brainstorm)

D1 scope = complete rollout (incl. admin invite/events forms) · D2 write-path E2E = build in-memory mocks, un-skip RSVP+Thanks (keep the repo's `_test-*-store` pattern; suite-wide MSW migration backlogged) · D3 form styling = `StatusEditor` verbatim (C7) · D4 modal = **native `<dialog>`+`.showModal()` (C8)** — `backdrop:bg-ink/50`, `bg-cream`/`border-ink` card, `font-display` title, `Pill` buttons (2026 standards upgrade) · D5 admin = metric-tile + table C9 · D6 `Pill` += `disabled:opacity-50` (BASE) + `danger` variant; `StatusEditor`/`GdprPanel` de-dup · D7 `ConsentModal` **upgraded to native `<dialog>`** — REVISED after 2026 standards validation (built-in Escape/`inert`/top-layer; no manual focus trap per WAI-APA consensus; closes the a11y gap in v0.9.1 with less code; consent logic untouched) · D8 doc home = this §19.1 (matches §18.1 point-release precedent).

### Hardenings

H104. `Pill` `disabled:opacity-50` + `danger` variant; no hand-rolled Pill look in `StatusEditor`/`GdprPanel` (source-scan). · H105. Form surfaces use C7 — no `neutral-*`/`gray-*`/`rounded border`; `bg-paper`/`bg-cream-deep` fields + `Pill` buttons (per-surface source-scan). · H106. `ConsentModal` uses native `<dialog>`+`.showModal()` (C8) — `backdrop:bg-ink/50`, explicit `aria-labelledby`, no manual focus-trap; old `role="dialog"` div gone. · H107. `/admin/health` tile + table tokenized (C9) — `border-l-ink` tile, `tabular-nums` number, no `rounded border`/`neutral-*`. · H108. `ProfileEditor` preview uses `.prose-warm`. · H109. RSVP + Thanks mocks carry the prod double-guard `!isProductionRuntime() && isE2EMode()` (source-scan + E2E green). · H110. `members/[slug]` empty-state path/slug rendered in `<code>` (mono affordance restored). · H111. `pnpm e2e` honest-green locally — routing assertions current; prod-only specs skip with a reason off `PLAYWRIGHT_BASE_URL`. **Carried:** H98 (`dark:` = 0 after reskin) · H103 (one `main#main` + one `<h1>` per recipe page; extend `PAGES`).

### Guards & out-of-scope

className-only on auth/consent/RBAC/save/write *logic*; the Phase-4 mock forks are the only behavioral additions and are strictly test-mode-guarded (H109; security-lane-gated). **Out:** suite-wide **MSW** server-side E2E interception (cleaner 2026 standard, but a consistency pass across all `_test-*-store` actions — own cycle; v0.9.1 keeps the in-app pattern), removal of dead `--color-neutral-*` token *definitions* in `globals.css` (separate dead-code task — harmless to leave), Tailwind v4 migration / real dark mode / CSP (unchanged from §19 non-goals).
