...
1. **Shared contracts + `Pill`** — add `disabled:opacity-50` (BASE) + a `danger` variant to `Pill`; document **C6** (form-field recipe = `StatusEditor` verbatim), **C7** (modal recipe), **C8** (metric-tile + table recipe); refactor `StatusEditor` (solid+dashed) + `GdprPanel` (solid+danger) to consume `Pill` and stop hand-rolling its look. Touches the shipped `Pill` → re-smoke the 4 hero surfaces.
2. **Member forms** — `/me/edit`+`ProfileEditor` (incl. `.prose-warm` preview, C2, replacing the no-op `prose prose-neutral`), `/consent`+`ConsentClient`+`ConsentModal` (C7 — native `<dialog>` upgrade), `/onboard`+`OnboardForm`+`/onboard/error`, `/no-access`.
3. **Admin** — `/admin/health` (C8 tile + tokenized trend table), `/admin/invite`+`InviteForm`+`InviteUrlDisplay`, `/admin/events/new`+`EventForm` (C6 across many fields — heaviest single reskin at 272 LOC).
