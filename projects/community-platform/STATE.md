...
**Chat 18 (DONE):** v0.3 plan-writing via `superpowers:writing-plans`. `projects/community-platform/v0.3.0-plan.md` at branch `chore/community-platform-v0-3-plan` (PR Draft). 5734 lines, 36 tasks across 4 phases (Phase 1 Foundation: 11, Phase 2 Read surfaces: 10, Phase 3 Write surfaces: 9, Phase 4 PWA+Closeout: 6), 229 checkboxes. Open spec questions locked with rationale: O1 = `ics` npm package (MIT, ~7KB); O2 = generated artifacts committed; O3+O4 = ADR-0012 documents discovery posture amendment to CONSTRAINTS line 12; O5 = `.json` not `.yaml`; O6 = SSG-only roster with sign-in CTA hint; O7 = profile re-read dedup; O8 = item_id shapes locked per item_type.
