...
- [ ] **Step 1: Full gate.** From `projects/community-platform/`: `pnpm tsc --noEmit && pnpm lint && pnpm test` — all three exit 0, zero test failures (coverage thresholds ≥80% hold; `lib/og-card.ts` is fully covered by Task 4.2; `lib/__generated__/*` churn from the `pretest` hook is benign noise).

- [ ] **Step 2: Manual render check — real persona.** Start `pnpm dev` (do NOT kill an already-running dev server — reuse it, or use `PORT=3100 pnpm dev` if 3000 is taken), then:

```bash
curl -s -o /tmp/og-anton.png -w "%{http_code} %{content_type}\n" http://localhost:3000/members/anton-safronov/opengraph-image
file /tmp/og-anton.png
open /tmp/og-anton.png
```

Expect `200 image/png`, `PNG image data, 1200 x 630`; visually: cream card, Geist 600 name, one-line bio, ≤5 uppercase mono expert tags, bottom-right `subploters` with amber `*`.

- [ ] **Step 3: Manual render check — fallback (H155).** Pick a roster member with NO attached persona (compare `ls ../../persona-builder/personas/` against `community/members/roster.md` slugs) and curl their card the same way → expect `200 image/png`, 1200×630, **name-only** card (no bio, no tags). If every member has a persona, temporarily set `persona_visible: false` on a local profile fixture is NOT needed — the H147 path is already unit-locked in `og-card.test.ts`; the missing-persona member exercises the H146 branch live.

- [ ] **Step 4: og:image meta injection sanity.** `curl -s http://localhost:3000/members/anton-safronov | grep -o 'og:image[^>]*'` → expect the auto-injected `og:image` meta pointing at the opengraph-image URL (file-convention wiring confirmed — no `generateMetadata` needed).

- [ ] **Step 5: Mark Phase 4 done.** Tick the phase checkbox in `v0.12.0-plan.md`; note in the plan's progress log that fonts were vendored from `geist@<installed-version>` `dist/fonts/` (or the adjusted source recorded in Task 4.1 Step 4). No commit unless the plan file changed: `git add docs/... && git commit -m "docs(community-platform): v0.12 plan — Phase 4 (OG card) complete"`.
