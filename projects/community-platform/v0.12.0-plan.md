...
- [x] **Step 1: Write the failing pair-contrast test (H156).** Create `tests/unit/contrast-pairs.test.ts`:
...
- [x] **Step 2: Write the failing token source-scan test.** Create `tests/unit/design-tokens.test.ts` (mirrors the existing `globals-tokens.test.ts` + `tailwind-config.test.ts` patterns):
...
- [x] **Step 3: Run both tests — expect RED.** `pnpm vitest run tests/unit/contrast-pairs.test.ts tests/unit/design-tokens.test.ts` — expected: in `contrast-pairs` 3/7 rows fail with `--color-ink-muted not found` / `--color-surface-soft not found` (the 4 passing rows pin pre-existing pairs — intended regression coverage); in `design-tokens` all 5 token rows + the focus-visible test + the tailwind test fail.

- [x] **Step 4: Add the tokens to `app/globals.css`.** Exact hunk (insert into `:root` after the v0.6 block):
...
- [x] **Step 5: Flip the global focus ring in `app/globals.css`.** Exact hunk:
...
- [x] **Step 6: Wire the Tailwind color tokens.** Exact hunk in `tailwind.config.ts`:
...
- [x] **Step 1: Write the failing parser tests.** Create `tests/unit/persona-sections.test.ts`. NOTE: the fixture intentionally uses the **typographic apostrophe U+2019** in “I’ve” (production `.public.md` files use it) and the literal slash heading `Competitor / substitute contexts`:
...
- [x] **Step 2: Run — expect RED.** `pnpm vitest run tests/unit/persona-sections.test.ts` — expected failure: `Failed to resolve import "@/lib/persona-sections"` (module does not exist).

- [x] **Step 3: Implement the parser.** Create `lib/persona-sections.ts` (RELATIVE imports only — none needed; pure module):
...
- [x] **Step 4: Run — expect GREEN.** `pnpm vitest run tests/unit/persona-sections.test.ts` — all tests pass.

- [x] **Step 5: Commit.** `git add lib/persona-sections.ts tests/unit/persona-sections.test.ts && git commit -m "feat(persona-redesign): persona-sections heading-skeleton parser (H148 never-throws)"`
...
- [x] **Step 1: Write the failing column tests.** Create `tests/unit/roster-columns.test.ts` — inline fixture mirrors BOTH real roster.md table shapes (Core organizers: Role column, NO Link column, `@antonsafronov (TBD)` telegram; Members: Link column present):
...
- [x] **Step 2: Run — expect RED.** `pnpm vitest run tests/unit/roster-columns.test.ts` — expected failures: `expect(received).toBeNull() — Received: undefined` (the fields don't exist on `RosterMember` yet; vitest transpiles without typechecking, so this fails at assertion, not compile).

- [x] **Step 3: Extend the interface + add the cell normalizer.** In `lib/roster.ts`, exact hunks:
...
- [x] **Step 4: Add per-table column detection (exactly like `githubColIndex`).** Three hunks in `parseRosterContent`:
...
- [x] **Step 5: Emit the new fields.** Final hunk:
...
- [x] **Step 6: Run — expect GREEN + regression sweep over existing roster suites.** `pnpm vitest run tests/unit/roster-columns.test.ts tests/unit/roster.test.ts tests/unit/roster-parse-content.test.ts tests/unit/roster-persona.test.ts` — all pass. (Audited: no existing test does `toEqual` against full `RosterMember` objects — the only roster `toEqual` maps to handle strings. If any full-object `toEqual` fails here, add `telegram/link/focus` to its expected objects rather than loosening the matcher.)

- [x] **Step 7: Confirm snapshot typing inherits cleanly.** `pnpm tsc --noEmit` — exit 0 (`MemberWithProfile` inherits the fields; the generated JSON is bridged by the existing `as unknown as ContentSnapshot` cast and regenerates on the next `pnpm test`/`pnpm build` via the pretest/prebuild hooks).

- [x] **Step 8: Commit.** `git add lib/roster.ts tests/unit/roster-columns.test.ts && git commit -m "feat(persona-redesign): roster Telegram/Link/Focus columns per-table (TBD/empty → null)"`
...
- [x] **Step 1: Write the failing overlap tests.** Create `tests/unit/persona-overlap.test.ts`:
...
- [x] **Step 2: Run — expect RED.** `pnpm vitest run tests/unit/persona-overlap.test.ts` — expected failure: `Failed to resolve import "@/lib/persona-overlap"`.

- [x] **Step 3: Add the 3 starter i18n keys.** Exact hunk in `lib/i18n/strings.ts`:
...
- [x] **Step 4: Implement `computeOverlap`.** Create `lib/persona-overlap.ts` (RELATIVE imports — GOTCHAS #10):
...
- [x] **Step 5: Run — expect GREEN.** `pnpm vitest run tests/unit/persona-overlap.test.ts` — all tests pass.

- [x] **Step 6: Phase-end gate (verification-before-completion).** From `projects/community-platform/`: `pnpm tsc --noEmit && pnpm lint && pnpm test` — expect: tsc exit 0, eslint clean, full suite green (≥1450 tests + the new Phase 1 files; `lib/__generated__/*` churn from the pretest hook is benign — do not commit it unless the diff is the new roster fields, in which case include the regenerated snapshot in the commit below).

- [x] **Step 7: Commit.** `git add lib/persona-overlap.ts lib/i18n/strings.ts tests/unit/persona-overlap.test.ts && git commit -m "feat(persona-redesign): overlap lens computeOverlap + starter templates (O1, H160)"`
...
- [x] **Step 1: Write the failing test** — create `tests/unit/member-kicker.test.tsx`:
...
- [x] **Step 2: Run it RED** — `pnpm vitest run tests/unit/member-kicker.test.tsx` → expect `Failed to load url @/app/components/MemberKicker`.
- [x] **Step 3: Implement** — create `app/components/MemberKicker.tsx`:
...
- [x] **Step 4: Run it GREEN** — `pnpm vitest run tests/unit/member-kicker.test.tsx` → 6 passed.
- [x] **Step 5: Commit** — `git add app/components/MemberKicker.tsx tests/unit/member-kicker.test.tsx && git commit -m "feat(persona-redesign): MemberKicker — kickers ARE the headings (H157)"`
...
- [x] **Step 1: Write the failing test** — create `tests/unit/expertise-ledger.test.tsx`:
...
- [x] **Step 2: Run it RED** — `pnpm vitest run tests/unit/expertise-ledger.test.tsx` → expect `Failed to load url @/app/components/ExpertiseLedger`.
- [x] **Step 3: Add the i18n keys** — `lib/i18n/strings.ts`, exact hunk (insert after the existing `members.detail.noProfilePathFmt` line; sentence-case values — uppercase comes from CSS per H157):
...
- [x] **Step 4: Implement** — create `app/components/ExpertiseLedger.tsx`:
...
- [x] **Step 5: Run it GREEN** — `pnpm vitest run tests/unit/expertise-ledger.test.tsx` → 9 passed.
- [x] **Step 6: Commit** — `git add app/components/ExpertiseLedger.tsx lib/i18n/strings.ts tests/unit/expertise-ledger.test.tsx && git commit -m "feat(persona-redesign): ExpertiseLedger — definition-list rows replace chips (D7)"`
...
- [x] **Step 1: Write the failing test** — create `tests/unit/first-question-posture.test.tsx`:
...
- [x] **Step 2: Run it RED** — `pnpm vitest run tests/unit/first-question-posture.test.tsx` → expect `Failed to load url @/app/components/FirstQuestionQuote`.
- [x] **Step 3: Add the i18n keys** — `lib/i18n/strings.ts`, exact hunk (chains after Task 2.2's keys; `askYours` must stay the last line — Task 2.4's hunk anchors on it):
...
- [x] **Step 4: Implement FirstQuestionQuote** — create `app/components/FirstQuestionQuote.tsx`:
...
- [x] **Step 5: Implement PostureLedger** — create `app/components/PostureLedger.tsx`:
...
- [x] **Step 6: Run it GREEN** — `pnpm vitest run tests/unit/first-question-posture.test.tsx` → 10 passed.
- [x] **Step 7: Commit** — `git add app/components/FirstQuestionQuote.tsx app/components/PostureLedger.tsx lib/i18n/strings.ts tests/unit/first-question-posture.test.tsx && git commit -m "feat(persona-redesign): FirstQuestionQuote pull-quote + PostureLedger bullish/skeptical rows"`
...
- [x] **Step 1: Write the failing test** — create `tests/unit/story-section.test.tsx`:
...
- [x] **Step 2: Run it RED** — `pnpm vitest run tests/unit/story-section.test.tsx` → expect `Failed to load url @/app/components/StorySection`.
- [x] **Step 3: Add the i18n keys** — `lib/i18n/strings.ts`, exact hunk (the story row + disposition labels are additions beyond the contract list — required by the every-string-through-`s()` rule / H67):
...
- [x] **Step 4: Implement** — create `app/components/StorySection.tsx`:
...
- [x] **Step 5: Run it GREEN** — `pnpm vitest run tests/unit/story-section.test.tsx` → 11 passed.
- [x] **Step 6: Commit** — `git add app/components/StorySection.tsx lib/i18n/strings.ts tests/unit/story-section.test.tsx && git commit -m "feat(persona-redesign): StorySection — JS-free fade + counted details rows (H158)"`
...
- [x] **Step 1: Write the failing test** — create `tests/unit/activity-line.test.tsx`:
...
- [x] **Step 2: Run it RED** — `pnpm vitest run tests/unit/activity-line.test.tsx` → expect `Failed to load url @/app/components/ActivityLine`.
- [x] **Step 3: Add the i18n keys** — `lib/i18n/strings.ts`, exact hunk (plural-only `Fmt` strings — accepted simplification, matches existing `Fmt` replace-pattern; D9's "1 status post" singular is forgone for token discipline):
...
- [x] **Step 4: Implement** — create `app/components/ActivityLine.tsx`:
...
- [x] **Step 5: Run it GREEN** — `pnpm vitest run tests/unit/activity-line.test.tsx` → 4 passed.
- [x] **Step 6: Commit** — `git add app/components/ActivityLine.tsx lib/i18n/strings.ts tests/unit/activity-line.test.tsx && git commit -m "feat(persona-redesign): ActivityLine — D9 mono stat footer replaces metric tiles"`
...
- [x] **Step 1: Write the failing CopyHandle test** — create `tests/unit/copy-handle.test.tsx`:
...
- [x] **Step 2: Write the failing page source-scan test** — create `tests/unit/member-page-dossier.test.ts`:
...
- [x] **Step 3: Run both RED** — `pnpm vitest run tests/unit/copy-handle.test.tsx tests/unit/member-page-dossier.test.ts` → CopyHandle fails on unresolved import; dossier scan fails on `max-w-3xl`/missing components.
- [x] **Step 4: Add the i18n keys** — `lib/i18n/strings.ts`, exact hunk (`metaLink` is an addition beyond the contract list, needed for the roster `link` column surfaced per §4.3):
...
- [x] **Step 5: Implement CopyHandle** — create `app/components/CopyHandle.tsx`:
...
- [x] **Step 6: Rewrite the page** — replace `app/members/[slug]/page.tsx` in full:
...
- [x] **Step 10: Reconcile remaining stale unit tests** — run `pnpm test`. For every failure in a pre-existing test file (locate candidates with `grep -rln 'members/\[slug\]' tests/`), apply this deterministic mapping — do NOT weaken assertions: `PersonaPanel` → the dossier components (`ExpertiseLedger`/`StorySection`); `ContributionCard` on this page → `ActivityLine`; `max-w-3xl` → `max-w-[688px]`; "Languages: en" text shape → ledger row (dt/dd); the page-level H147 test keeps passing as-is (`persona_visible !== false` is preserved verbatim). Re-run `pnpm test` until green.
- [x] **Step 11: Commit** — `git add -A app/members/[slug]/page.tsx app/components/CopyHandle.tsx lib/i18n/strings.ts e2e/persona-attach.spec.ts tests/ && git commit -m "feat(persona-redesign): typeset-dossier member page (spec §3 order, O2/O5 locks); delete PersonaPanel
...
- [x] **Step 1: Heading-outline test (H157)** — create `tests/unit/member-page-heading-outline.test.tsx`:
...
- [x] **Step 2: Hit-area source scan (H159)** — create `tests/unit/member-page-hit-areas.test.ts`:
...
- [x] **Step 3: Details-constraints source scan (H158)** — create `tests/unit/member-details-constraints.test.ts`:
...
- [x] **Step 4: Run all three** — `pnpm vitest run tests/unit/member-page-heading-outline.test.tsx tests/unit/member-page-hit-areas.test.ts tests/unit/member-details-constraints.test.ts` → all pass (these are regression locks over Tasks 2.1–2.6 output; if any fails, fix the component, not the test). Note: the VoiceOver (macOS + iOS) pass required by spec §3 happens at ship smoke, not in this phase.
- [x] **Step 5: Commit** — `git add tests/unit/member-page-heading-outline.test.tsx tests/unit/member-page-hit-areas.test.ts tests/unit/member-details-constraints.test.ts && git commit -m "test(persona-redesign): a11y regression locks — H157 heading outline, H158 details constraints, H159 hit areas"`
...
- [x] **Step 1: Write the contract-lock test** — create `tests/unit/i18n-v0-12-keys.test.ts` (regression pin, not RED/GREEN — keys were added incrementally in Tasks 2.2–2.6; this locks the exact contract values):
...
- [x] **Step 2: Run it** — `pnpm vitest run tests/unit/i18n-v0-12-keys.test.ts` → 16 passed. If any key fails, fix the value in `lib/i18n/strings.ts` to the contract value (the contract wins), re-run.
- [x] **Step 3: Run the inline-strings scan** — `pnpm h67:scan` → expect a clean exit (no hardcoded UI strings introduced by Phase 2 components; the decorative `// `, ` · `, `✓`, `★`, `*`, `▾` glyphs are non-translatable punctuation, same posture as existing surfaces).
- [x] **Step 4: Commit** — `git add tests/unit/i18n-v0-12-keys.test.ts lib/i18n/strings.ts && git commit -m "test(persona-redesign): lock v0.12 Phase 2 i18n key contract"`
...
- [x] **Step 1: Type-check** — `pnpm tsc --noEmit` → exit 0. Most likely failure: `member.telegram`/`link`/`focus` missing on `MemberWithProfile` ⇒ Phase 1 not fully landed — stop and fix Phase 1, not the page.
- [x] **Step 2: Lint** — `pnpm lint` → exit 0 (the `void failurePatterns` pattern matches the existing `Avatar.tsx` precedent; if `no-unused-expressions` still fires, switch to `props`-object access with a destructure of only the rendered fields).
- [x] **Step 3: Full unit suite** — `pnpm test` → 0 failures, coverage thresholds (80/80/80/80) hold. New files this phase are all under the coverage `include` globs (`app/components/**`).
- [x] **Step 4: Verify deletions are clean** — `grep -rn "PersonaPanel" app/ lib/ tests/ e2e/` → zero hits.
- [x] **Step 5: Commit only if Steps 1–4 forced changes** — `git add <touched files> && git commit -m "fix(persona-redesign): phase 2 gate fixes (tsc/lint/test)"`. Otherwise no commit; phase complete. Playwright E2E (`pnpm e2e`, includes the amended `persona-attach.spec.ts`) and the VoiceOver pass run at the plan's pre-ship smoke, not here.
...
- [x] **Step 1: Write the failing RTL test** — create `tests/unit/overlap-lens.test.tsx`:
...
- [x] **Step 2: Run — expect RED.** `pnpm vitest run tests/unit/overlap-lens.test.tsx` → fails with `Failed to resolve import "@/app/components/OverlapLens"`.

- [x] **Step 3: Add the five lens display keys** to `lib/i18n/strings.ts`, inside the `strings` object, as a new section directly after the `members.detail.*` block (insertion — surrounding lines unchanged):
...
- [x] **Step 4: Implement the component** — create `app/components/OverlapLens.tsx` (Server Component; no `"use client"`):
...
- [x] **Step 5: Run — expect GREEN.** `pnpm vitest run tests/unit/overlap-lens.test.tsx` → 10 passed.
...
- [x] **Step 1: Failing unit test for the content gate** — create `tests/unit/persona-overlap-has-content.test.ts`:
...
- [x] **Step 2: Run — expect RED.** `pnpm vitest run tests/unit/persona-overlap-has-content.test.ts` → fails (`overlapHasContent is not a function` / no export).

- [x] **Step 3: Implement the gate helper** — append to `lib/persona-overlap.ts` (relative imports only inside lib/ — this adds no import):
...
- [x] **Step 4: Run — expect GREEN.** `pnpm vitest run tests/unit/persona-overlap-has-content.test.ts` → 4 passed.

- [x] **Step 5: Failing integration test for the four gating conditions** — create `tests/integration/member-page-overlap.test.tsx`:
...
- [x] **Step 6: Run — expect RED on the happy path.** `pnpm vitest run tests/integration/member-page-overlap.test.tsx` → gates 1–4 pass vacuously (lens not wired), final test fails with `Unable to find an accessible element with the role "heading" … overlap: you and Nadia K`.

- [x] **Step 7: Wire the page** — `app/members/[slug]/page.tsx`, three hunks (anchor on shown text; Phase-2 line numbers may drift):
...
- [x] **Step 8: Run — expect GREEN.** `pnpm vitest run tests/integration/member-page-overlap.test.tsx tests/unit/persona-overlap-has-content.test.ts` → 9 passed. If the import of `@/lib/content-snapshot` in the test errors on a missing export, add that name to the mock factory (returning `[]`/`{}` equivalents) — do not weaken the gates.
...
- [x] **Step 1: Failing CopyHandle unit test** — create `tests/unit/copy-handle.test.tsx`:
...
- [x] **Step 2: Run — expect RED.** `pnpm vitest run tests/unit/copy-handle.test.tsx` → `Failed to resolve import "@/app/components/CopyHandle"`.

- [x] **Step 3: Add the three i18n keys** to `lib/i18n/strings.ts`, directly after the `members.detail.viewCard` entry (Phase 2) — insertion:
...
- [x] **Step 4: Implement CopyHandle** — create `app/components/CopyHandle.tsx`:
...
- [x] **Step 5: Run — expect GREEN.** `pnpm vitest run tests/unit/copy-handle.test.tsx` → 4 passed.

- [x] **Step 6: Failing page-wiring test** — create `tests/integration/member-page-ask-about.test.tsx`:
...
- [x] **Step 7: Run — expect RED.** `pnpm vitest run tests/integration/member-page-ask-about.test.tsx` → both tests fail (no ask link, no copy button).

- [x] **Step 8: Wire the actions row** — `app/members/[slug]/page.tsx`. Add to the imports block: `import { CopyHandle } from "@/app/components/CopyHandle";`. Then in the actions row (anchor on `s("members.detail.viewCard")`), old:
...
- [x] **Step 9: Run — expect GREEN, plus no regressions.** `pnpm vitest run tests/integration/member-page-ask-about.test.tsx tests/integration/member-page-overlap.test.tsx` → 7 passed.
...
- [x] **Step 1: Targeted suite green.** `pnpm vitest run tests/unit/overlap-lens.test.tsx tests/unit/persona-overlap-has-content.test.ts tests/unit/copy-handle.test.tsx tests/integration/member-page-overlap.test.tsx tests/integration/member-page-ask-about.test.tsx` → 0 failures.
- [x] **Step 2: Full gate.** `pnpm tsc --noEmit && pnpm lint && pnpm test` → all exit 0 (full unit + integration suite; `pretest` regenerates `lib/__generated__/*` — benign).
- [x] **Step 3: Invariant greps.** `grep -n "dark:" app/components/OverlapLens.tsx app/components/CopyHandle.tsx` → no matches (no-dark rule); `grep -nE "console\.|posthog|fetch\(" app/components/OverlapLens.tsx` → no matches (H160: lens render is side-effect free); `grep -c "text-dust\|text-accent-700" app/components/OverlapLens.tsx` → 0 (H156 on surface-soft).
- [x] **Step 4: Clean tree.** `git checkout -- lib/__generated__` to drop pretest snapshot noise; `git status` shows no other unstaged changes. Do not proceed to Phase 4 until Steps 1–3 are green.
...
- [x] **Step 1: Write the failing font-presence + size-budget test.** Create `tests/unit/og-fonts.test.ts`:
...
- [x] **Step 2: Run it — expect RED.** `pnpm vitest run tests/unit/og-fonts.test.ts` → 4 failures (`existsSync` false; `statSync` throws ENOENT). Confirm the failure is "file missing", nothing else.

- [x] **Step 3: Install the geist package (prod dep).** `pnpm add geist` (from `projects/community-platform/`). Expect `dependencies.geist` added to `package.json`.

- [x] **Step 4: Locate the TTFs and copy them.** Run the discovery first — paths are verified, never assumed:
...
- [x] **Step 5: Run the test — expect GREEN.** `pnpm vitest run tests/unit/og-fonts.test.ts` → 4 pass.
...
- [x] **Step 1: Write the failing card-model test (H155 gate logic).** Create `tests/unit/og-card.test.ts`:
...
- [x] **Step 2: Run it — expect RED.** `pnpm vitest run tests/unit/og-card.test.ts` → fails: `Cannot find module '@/lib/og-card'`.

- [x] **Step 3: Implement `lib/og-card.ts` (RELATIVE imports — GOTCHAS #10).**
...
- [x] **Step 4: Run it — expect GREEN.** `pnpm vitest run tests/unit/og-card.test.ts` → 7 pass.

- [x] **Step 5: Write the failing route source-scan test (H155 invariants).** Create `tests/unit/og-image-route.test.ts`:
...
- [x] **Step 6: Run it — expect RED.** `pnpm vitest run tests/unit/og-image-route.test.ts` → fails: ENOENT reading `app/members/[slug]/opengraph-image.tsx`.

- [x] **Step 7: Implement the route.** Create `app/members/[slug]/opengraph-image.tsx` (app/ file ⇒ `@/` aliases; satori JSX = inline styles only, every multi-child div gets explicit `display:flex`):
...
- [x] **Step 8: Run both — expect GREEN.** `pnpm vitest run tests/unit/og-image-route.test.ts tests/unit/og-card.test.ts` → all pass.
...
- [x] **Step 1: Write the failing source-scan test.** Create `tests/unit/member-page-card-link.test.ts`:
...
- [x] **Step 2: Run it — expect RED.** `pnpm vitest run tests/unit/member-page-card-link.test.ts` → 3 failures (no `/opengraph-image` in page source; no `CardEmbedSnippet` in me/edit).

- [x] **Step 3: Add the i18n keys.** In `lib/i18n/strings.ts` — exact hunks. After the `members.detail.noProfilePathFmt` line (Phase 2/3 may have appended more `members.detail.*` keys below it — append after the last one; old→new shown against the known anchor):
...
- [x] **Step 5: Write the failing CardEmbedSnippet render test.** Create `tests/unit/card-embed-snippet.test.tsx`:
...
- [x] **Step 6: Run it — expect RED.** `pnpm vitest run tests/unit/card-embed-snippet.test.tsx` → fails: `Cannot find module '@/app/components/CardEmbedSnippet'`.

- [x] **Step 7: Implement the client component.** Create `app/components/CardEmbedSnippet.tsx` (CopyHandle clipboard pattern; button = Pill per platform rule):
...
- [x] **Step 8: Wire it into /me/edit.** In `app/me/edit/page.tsx` — exact hunks. Imports (old→new):
...
- [x] **Step 9: Run all task tests — expect GREEN.** `pnpm vitest run tests/unit/card-embed-snippet.test.tsx tests/unit/member-page-card-link.test.ts` → all pass.
...
- [x] **Step 1: Write the failing config-scan test.** Append to `tests/unit/og-image-route.test.ts` (old→new — after the closing `});` of the `describe("opengraph-image route (H155)")` block, add):
...
- [x] **Step 2: Run it — expect RED.** `pnpm vitest run tests/unit/og-image-route.test.ts` → 1 new failure (no `outputFileTracingIncludes` in config).

- [x] **Step 3: Apply the next.config.ts hunk.** Old→new:
...
- [x] **Step 4: Run it — expect GREEN.** `pnpm vitest run tests/unit/og-image-route.test.ts` → all pass.

- [x] **Step 5: Verify the trace lands in a real build.** `pnpm build` (exit 0), then:
...
- [x] **Step 5: Mark Phase 4 done.** Fonts vendored from `geist@1.7.2` at `dist/fonts/geist-sans/Geist-SemiBold.ttf` and `dist/fonts/geist-mono/GeistMono-Regular.ttf` (exact paths per plan). Both 128KB/149KB (under 300KB budget). Plan-vs-reality mismatch fixed: the proxy was auth-gating the OG route (H155 requires it unauthenticated for scrapers/camo). First fix (`f94f071`) over-broadly exposed the whole `/members/` prefix citing a nonexistent "D12 amendment" — **corrected at orchestrator review**: only `^/members/[^/]+/opengraph-image$` is public (`OG_IMAGE_PATH` in `proxy.ts`); `/members` + `/members/[slug]` stay auth-gated per the ADR-0012/0014 routing model (v0.9.1-verified). Regression tests pin both directions.
...
- [x] **Step 1: Write the failing test suite** — create `tests/unit/persona-fetch.test.ts`:
...
- [x] **Step 2: Run RED** — `pnpm vitest run tests/unit/persona-fetch.test.ts` → expect failure `Failed to resolve import "@/lib/persona-fetch"` (module does not exist yet).
- [x] **Step 3: Implement** — create `lib/persona-fetch.ts` (RELATIVE import — GOTCHAS #10):
...
- [x] **Step 4: Run GREEN** — `pnpm vitest run tests/unit/persona-fetch.test.ts` → expect 18 passing tests, 0 failures.
- [x] **Step 5: Commit** — `git add lib/persona-fetch.ts tests/unit/persona-fetch.test.ts && git commit -m "feat(persona-redesign): add lib/persona-fetch — H151 URL guard + H152 streamed 64KB byte cap"`
...
- [x] **Step 1: Write the failing schema tests** — in `tests/unit/persona-editor.test.ts`, replace the `SavePersonaSchema` describe block. Old:
...
- [x] **Step 2: Write the failing client-cap RTL test** — create `tests/unit/persona-editor-ui.test.tsx`:
...
- [x] **Step 3: Run RED** — `pnpm vitest run tests/unit/persona-editor.test.ts tests/unit/persona-editor-ui.test.tsx` → expect 2 failures: the 65,538-byte CJK safeParse returns `success: true` under the old `.max()`, and the RTL test finds no tooLarge message.
- [x] **Step 4: Implement schema + union** — in `lib/persona-editor.ts`. Hunk 1 (imports), old:
...
- [x] **Step 5: Implement client check** — in `app/components/PersonaEditor.tsx`, old:
...
- [x] **Step 6: Run GREEN + typecheck** — `pnpm vitest run tests/unit/persona-editor.test.ts tests/unit/persona-editor-ui.test.tsx` → all pass; `pnpm tsc --noEmit` → exit 0 (confirms the union literals `"invalid_url" | "fetch_failed" | "fetch_too_large"` are now assignable to `PersonaSaveError`).
- [x] **Step 7: Commit** — `git add lib/persona-editor.ts app/components/PersonaEditor.tsx tests/unit/persona-editor.test.ts tests/unit/persona-editor-ui.test.tsx && git commit -m "feat(persona-redesign): byte-accurate 64KB cap parity via TextEncoder in schema + client (H152)"`
...
- [x] **Step 1: Write the failing schema test** — create `tests/unit/profile-frontmatter-source-url.test.ts`:
...
- [x] **Step 2: Write the failing action tests** — in `tests/integration/save-persona.test.ts`. Add the persona-fetch partial mock after the existing `vi.mock("@/lib/log", …)` line (the factory closes over `fetchPersonaMock` lazily — same pattern as `authMock`):
...
- [x] **Step 5: Implement the action** — replace `app/actions/save-persona.ts` with:
...
- [x] **Step 6: Run GREEN** — `pnpm vitest run tests/integration/save-persona.test.ts tests/unit/profile-frontmatter-source-url.test.ts tests/integration/save-profile.test.ts tests/unit/profile-editor.test.ts` → all pass (the two profile suites confirm the schema field is non-breaking). Then `pnpm tsc --noEmit` → exit 0.
- [x] **Step 7: Commit** — `git add app/actions/save-persona.ts lib/profile-editor.ts tests/integration/save-persona.test.ts tests/unit/profile-frontmatter-source-url.test.ts && git commit -m "feat(persona-redesign): savePersona source_url branch — H154 single pipeline + persona_source_url profile commit (H151, O4)"`
...
- [x] **Step 1: Write the failing test suite** — create `tests/integration/resync-persona.test.ts`. `@/lib/persona-fetch` stays REAL here; only the global `fetch` is stubbed, so the H151 guard inside `fetchPersonaFromUrl` is exercised for real:
...
- [x] **Step 2: Run RED** — `pnpm vitest run tests/integration/resync-persona.test.ts` → expect `Failed to resolve import "@/app/actions/resync-persona"`.
- [x] **Step 3: Implement** — create `app/actions/resync-persona.ts`:
...
- [x] **Step 4: Run GREEN** — `pnpm vitest run tests/integration/resync-persona.test.ts` → 6 passing. Then `pnpm tsc --noEmit` → exit 0.
- [x] **Step 5: Commit** — `git add app/actions/resync-persona.ts tests/integration/resync-persona.test.ts && git commit -m "feat(persona-redesign): resync-persona server action — stored-URL TOCTOU re-validation (H153)"`
...
- [x] **Step 1: Write the invariant-lock test** — append inside the top-level `describe("H146: POST /api/me/delete — persona file erasure", …)` in `tests/integration/me-delete-persona.test.ts` (after the third `it`, before the E2E describe):
...
- [x] **Step 2: Run — expected GREEN immediately** — `pnpm vitest run tests/integration/me-delete-persona.test.ts` → all pass. This is a deliberate characterization test (deviation from RED→GREEN, called out here): the deletion behavior already exists; the test locks that `persona_source_url` erasure rides on it.
- [x] **Step 3: Document the invariant in the route** — in `app/api/me/delete/route.ts`, old:
...
- [x] **Step 4: Re-run + commit** — `pnpm vitest run tests/integration/me-delete-persona.test.ts` → all pass. `git add app/api/me/delete/route.ts tests/integration/me-delete-persona.test.ts && git commit -m "test(persona-redesign): lock H146 erasure of persona_source_url via profile deletion in /api/me/delete"`
...
- [x] **Step 1: Add the i18n keys** — in `lib/i18n/strings.ts`, old:
...
- [x] **Step 2: Write the failing RTL suite** — replace `tests/unit/persona-editor-ui.test.tsx` (created small in Task 5.2) with the full final file:
...
- [x] **Step 3: Run RED** — `pnpm vitest run tests/unit/persona-editor-ui.test.tsx` → expect failures: no URL field, no Fetch & attach button, source-scan finds the v0.11.1 literals.
- [x] **Step 4: Implement the component** — replace `app/components/PersonaEditor.tsx` with:
...
- [x] **Step 5: Wire `initialSourceUrl` from the profile frontmatter** — in `app/me/edit/page.tsx`. Hunk 1, old:
...
- [x] **Step 6: Run GREEN** — `pnpm vitest run tests/unit/persona-editor-ui.test.tsx tests/unit/persona-editor.test.ts` → all pass. Then `pnpm tsc --noEmit && pnpm lint` → exit 0 (lint also re-runs the H67 inline-string scan via config).
- [x] **Step 7: Commit** — `git add app/components/PersonaEditor.tsx app/me/edit/page.tsx lib/i18n/strings.ts tests/unit/persona-editor-ui.test.tsx && git commit -m "feat(persona-redesign): PersonaEditor URL attach + re-sync pills + full status-string i18n flip"`
...
- [x] **Step 1: Extend the spec** — append to `e2e/persona-attach.spec.ts` (reuses the existing `MEMBER_SLUG`/`MEMBER_HANDLE` consts and `loginAs`/`resetPersonaStore`/`seedProfileStore` helpers):
...
- [x] **Step 2: Run — expected GREEN** — `pnpm e2e e2e/persona-attach.spec.ts` → all tests in the file pass (the two existing 5.1 tests + the two new 5.7 tests). This task is end-to-end verification of work landed in 5.3 + 5.6 — there is no RED step; if either new test fails, treat it as a real defect in the action/UI wiring and debug via `superpowers:systematic-debugging`, not by loosening the spec.
- [x] **Step 3: Commit** — `git add e2e/persona-attach.spec.ts && git commit -m "test(persona-redesign): E2E URL-attach happy path + H151 guard negative via fixture fork"`

### Task 5.8: Phase 5 gate

**Files:** none (verification only)

- [x] **Step 1: Full gate** — `pnpm tsc --noEmit && pnpm lint && pnpm test` → all three exit 0; unit/integration suite fully green (expect ~+45 tests vs. the pre-phase baseline; zero skips added by this phase).
- [x] **Step 2: Targeted E2E re-run** — `pnpm e2e e2e/persona-attach.spec.ts` → 4/4 pass.
- [x] **Step 3: Coverage spot-check** — `pnpm test:coverage` → thresholds (80/80/80/80) hold; `lib/persona-fetch.ts` and `app/actions/resync-persona.ts` both appear with ≥80% line coverage.
- [x] **Step 4: Hygiene** — `git status --porcelain` shows only `lib/__generated__/*` pretest noise (do not commit it); confirm no `dark:` variants were introduced: `grep -rn "dark:" app/components/PersonaEditor.tsx app/me/edit/page.tsx` → no matches. Phase 5 done — hardenings H151–H154 implemented and test-locked; H146/O4 erasure locked; v0.11.1 hardcoded editor strings eliminated.
