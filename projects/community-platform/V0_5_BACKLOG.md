...
## From chat-47 (v0.9.1 brainstorm — 2026-05-27)

### Full MSW migration of the E2E mock layer (own version)

- **Migrate the entire `_test-*-store` family to MSW (Mock Service Worker) server-side interception** — surfaced during the v0.9.1 standards validation (chat-47). The 2026-recommended pattern for server-action E2E is "mock the external boundary, not the action": MSW (`instrumentation.ts` + `setupServer`) intercepts the GitHub REST calls so each write action runs **unchanged** — removing all in-app `_test-*-store.ts` files + every `isE2EMode`/`NEXT_PUBLIC_E2E_MODE` fork from production code, and giving higher-fidelity coverage (the real GitHub-call path is exercised, just with a mocked HTTP response). **Why defer:** v0.9.1 deliberately keeps the consistent in-app pattern (4 proven actions — status/profile/invitation/consent — plus the 2 new rsvp/thank); a piecemeal swap would fracture the suite. This is a **test-infra cycle** touching working/shipped code + their E2E specs, a new `msw` dependency, and non-trivial Next 16 server-side wiring (`fetchLoopback`/`bypass`). **Anton confirmed defer 2026-05-27** ("pick up full-MSW-migration as its own version later"). **Re-evaluate:** its own brainstorm → spec → plan cycle (candidate v0.9.2 or v0.10); pairs well with any future write-action work. Rationale + sources in the `docs/specs/2026-05-27-community-platform-v0-9-1-forms-admin-design.md` appendix.

---

*Updated 2026-05-27 (chat-47) — added the full-MSW-migration deferral from the v0.9.1 brainstorm standards validation (Anton-confirmed defer). Updated 2026-05-27 (chat-45) — v0.9.0 shipped: marked v0.9 redesign-completion + dark: landmine item DONE; added v0.9.1 forms/admin reskin + write-path E2E item. Updated 2026-05-27 (chat-44) with v0.9 redesign-completion + `dark:` landmine findings (27 files) from v0.8.1 close. Updated 2026-05-20 with chat-31 admin event-creation UI brainstorm seed. Updated 2026-05-20 with chat-29 admin event-creation UI deferral. Updated 2026-05-18 with chat-22 brainstorm deferrals. Original scaffold drafted 2026-05-17 in chat-21-prep close-out. Lives at `projects/community-platform/V0_5_BACKLOG.md` per "lean handoffs + deferred items get a home" convention.*
