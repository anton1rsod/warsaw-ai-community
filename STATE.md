...
## Latest handoff
- `docs/specs/2026-06-10-community-platform-v0-11-1-merge-handoff.md` — **DONE** (v0.11.1 SHIPPED 2026-06-10 at `616ba6f`; tag `community-platform-v0.11.1`; ADR-0019 Accepted; slug `anton-s`→`anton-safronov` aligned). No open community-platform handoff. **Next scope = the v0.11.2 fast-follows** (per-IP rate-limit on redemption · signed fresh-member bridge cookie · persistent active-invite registry · meeting-token `iat` · revoke auto-retry · persona-builder→roster-slug convention) — Anton-paced; no formal handoff written yet.
