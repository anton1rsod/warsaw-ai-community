...
- **Community Platform v0.11.1 — persona attach + rich card: SHIPPED 2026-06-10.** Bundle B (the v0.11.0-deferred persona fast-follow). Members self-attach a persona (paste/`.md` upload) with consent + a `persona_visible` hide toggle; renders as a rich card (controlled-vocab chips + languages + full `.public.md` body via SafeHtml; H138 renders `.public.md` ONLY). PR #55 squash-merged at `616ba6f` (Anton-authorized override of the 6/12 retro gate); tag `community-platform-v0.11.1`; ADR-0019 Accepted. subagent-driven (one Sonnet impl/phase, 5 phases, 18 commits); **1582 unit/int + e2e 2/2**; security(H150)+ts+code review **0C** (2 HIGH fixed); `lib/persona.ts`+`lib/persona-editor.ts` 100%; CI green 2m4s. Slug aligned `anton-s`→`anton-safronov` so Anton's card renders. See `projects/community-platform/STATE.md` + CHANGELOG `[0.11.1]`.
