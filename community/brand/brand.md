...
- **v1.2 — IN DESIGN (chat-40+, scope-locked chat-39 2026-05-25).** Three intertwined moves: (1) PL monogram standalone mark retired — too on-the-nose Poland reference; works against the multi-location architecture (§5) that deliberately equalizes cities; replacement candidates include a PSA monogram (P+S+A interlocked) or a money/growth glyph. (2) Trailing `*` qualifier retired — replaced with a money/growth emoji or minimal glyph. (3) Location identification (§4.3 city stamp + §4.4 formal entity lockup + §4.5 location sub-marks) to wire into community-platform (currently the platform Header only renders the wordmark). Master wordmark typographic form ("Subploters" Geist SemiBold path-drawn, ink), tagline ("Every venture is a subplot."), color palette, typography rules, city stamp visual treatment, and naming architecture all stay locked from v1.1. Handoff: `docs/specs/2026-05-25-subploters-brand-followups-handoff.md`.
