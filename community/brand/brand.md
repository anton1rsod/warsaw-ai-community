...
- **v1.2 — IN DESIGN (chat-41, Path Z locked 2026-05-25).** Path Z (wordmark-only, Stripe/Substack lineage) confirmed by Anton at chat-40 close after 3 rounds × 16 candidates × 0 winners. Chat-41 to brainstorm three sub-questions: (Q1) favicon slot when there's no standalone mark — "S" extracted (Substack-style) / "Sub" fragment / standalone `*` / new minimal glyph; (Q2) trailing `*` — retired entirely or kept as wordmark typographic accent; (Q3) platform location wire-in scope (§4.3/§4.4/§4.5) — Header / Footer / About / all three. Then writing-plans for asset regeneration + platform wire-in. Chat-40 archive: `community/brand/explorations/chat-40/` (16 rejected candidates + combined anti-patterns). Handoff: `docs/specs/2026-05-25-subploters-brand-v1-2-chat40-closeout-handoff.md`. Until v1.2 lands, the v1.1 PL monogram standalone mark + trailing `*` qualifier remain LIVE in `community/brand/assets/`.
