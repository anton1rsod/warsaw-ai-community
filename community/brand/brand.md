...
- **v1.2 — STILL OPEN (chat-40 attempt 1 closed without landing, 2026-05-25).** Three intertwined moves remain on the table: (1) replacement for the retired PL standalone mark, (2) replacement for the retired trailing `*` qualifier, (3) platform wire-in of location identification (§4.3/§4.4/§4.5). Chat-40 ran three rounds of brainstorming with the chat-38 path-drawing pipeline (opentype.js + Geist SemiBold) and produced 16 candidates across PSA monogram architectures, badge variants, and abstract single-shape glyphs — **all rejected**. Archive: `community/brand/explorations/chat-40/` documents what was tried + the combined anti-pattern list. Chat-41 to re-aim, no candidates in first response. Five paths surfaced (external designer / restore PL + defer to v2 / wordmark-only / mood board first / direction brainstorm only). Handoff: `docs/specs/2026-05-25-subploters-brand-v1-2-chat40-closeout-handoff.md`. Until v1.2 lands, the v1.1 PL monogram standalone mark + trailing `*` qualifier remain LIVE in `community/brand/assets/` — chat-40 did not retire them in code, only in the chat-39 design scope-pivot intent.
