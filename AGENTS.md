# AGENTS.md — Warsaw AI Community

> Canonical agent instructions (cross-tool open standard). Claude Code reads `CLAUDE.md`, which imports this file.
...
2. **`projects/<name>/CLAUDE.md`** (or `AGENTS.md`) if working in a sub-project — has its own read order.
...
Don't pre-read on every turn. Read by need.
...
- **Co-founder working agreement** → `docs/playbooks/collaboration.md` (ownership/DRI, branch & PR policy, handoff + anti-staleness rule).
